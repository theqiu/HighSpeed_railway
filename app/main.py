...
DEFAULT_DATA = BASE_DIR / "data" / "2025年6-9月.csv"
FALLBACK_SAMPLE = BASE_DIR / "data" / "sample_inspections.csv"
...
def load_data(file: Optional[BytesIO]) -> pd.DataFrame:
    ...
    if file is None:
        dataset_path = None
        for candidate in (DEFAULT_DATA, FALLBACK_SAMPLE):
            if candidate.exists():
        ...
        if dataset_path is None:
            raise FileNotFoundError(
                "默认数据文件未找到，请上传数据集或将其放置在 data 目录下。"
            )
...
def render_process_matrix(df: pd.DataFrame, artifacts: dict) -> None:
    ...
    if not required.issubset(df.columns):
        ...
        st.info(f"当前数据缺少绘制流程矩阵所需的列：{missing}")
        return

    working = df.dropna(subset=["opno", "xb005"]).copy()
    if working.empty:
        st.info("所选数据中没有可用于构建流程矩阵的记录。")
        return

    working["opno_str"] = working["opno"].astype(str)
    working["xb005_str"] = working["xb005"].astype(str)

    opno_list = (
        working["opno_str"].drop_duplicates().sort_values(kind="mergesort").tolist()
    )
    wheel_list = (
        working["xb005_str"].drop_duplicates().sort_values(kind="mergesort").tolist()
    )

    total_rows = len(opno_list)
    total_cols = len(wheel_list)
    ...
    row_window = max(1, min(row_window, total_rows))
    col_window = max(1, min(col_window, total_cols))
    ...
    with slider_columns[1]:
        ...
        visible_opnos = opno_list[row_lower:row_upper]
        visible_wheels = wheel_list[col_lower:col_upper]

        visible_mask = working["opno_str"].isin(visible_opnos) & working["xb005_str"].isin(
            visible_wheels
        )
        subset = working.loc[visible_mask]

        if subset.empty:
            visible = pd.DataFrame(
                0,
                index=pd.Index(visible_opnos, name="opno"),
                columns=visible_wheels,
            )
        else:
            visible = stage_presence_matrix(subset)
            visible.index = visible.index.astype(str)
            visible = visible.reindex(index=visible_opnos, columns=visible_wheels, fill_value=0)

        visible_rows = len(visible.index)
        visible_cols = len(visible.columns)
        figure_height = max(400, visible_rows * 24 if visible_rows else 400)
        figure_width = max(700, visible_cols * 60 if visible_cols else 700)
    ...
    matrix_table = visible.reset_index().rename(columns={visible.index.name or "index": "opno"})
        ...
        if selected_wheel:
            ...
            if violations.empty:
                ...
                for _, violation in violations.iterrows():
                    group_label = (
                        violation.get("panel_group")
                        or violation.get("panel_name")
                        or violation.get("panel_side")
                        or "该轮对"
                    )
                    flagged_sections = violation.get("violating_sections")
                    if isinstance(flagged_sections, str) and flagged_sections:
                        st.error(
                            f"{group_label}存在由内向外不递减的截面（{flagged_sections}），请复核加工过程。"
                        )
                    else:
